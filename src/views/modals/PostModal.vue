<template>
  ...
  <AppModal v-model="openEditModal">
    ...
    <el-tabs v-model="activeName" class="" @tab-click="handleClick">
      <form @submit.prevent="addPost" class="flex flex-col gap-5">
...
const addPost = async () => {
  if (!forms.value.category_id) {
    return danger("category is required");
  }
  loading.value = true;
  if (forms.value?.id) {
    const res = await store.updatePost(forms.value?.id, forms.value);
    store.getPosts();
    if (!res?.error) {
    ...
    const res = await store.addPost(forms.value);
    store.getPosts();
    if (!res?.error) {
...
const confirm = async () => {
  loading.value = true;
  const res = await store.deletePost(ID.value);
  store.getPosts();
  loading.value = false;
  if (!res?.error) {
